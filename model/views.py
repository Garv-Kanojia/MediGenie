...
def upload_image(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            predicted_labels = (predictions >= 0.3).astype(int)  # Threshold for prediction is 0.5
            # print(predictions[0])
            ...
            significant_predictions = [(label, prob) for label, prob in prediction_list if prob > 0.3]
...
def url_image(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Filter predictions above threshold (0.3) and sort by probability in descending order
            significant_predictions = [(label, prob) for label, prob in prediction_list if prob > 0.3]
            significant_predictions.sort(key=lambda x: x[1], reverse=True)
